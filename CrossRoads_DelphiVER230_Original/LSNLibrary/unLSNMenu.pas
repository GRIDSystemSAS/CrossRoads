...
uses
  ...
  FMX.Menus, FMX.Effects, FMX.Layouts;
...
begin
  ...
  FScreenRect.Position.X := FParent.Width + 10;
  //
  FScreenRect.AnimateFloatDelay('Position.X',0,1.5,0.5,TAnimationType.atIn,TInterpolationType.itQuartic);
end;

procedure TLSNSubMenu.HideScreen;
begin
  FScreenRect.AnimateFloatDelay('Scale.X',0.8,0.5,0,TAnimationType.atIn,TInterpolationType.itLinear);
  FScreenRect.AnimateFloatDelay('Scale.Y',0.8,0.5,0,TAnimationType.atIn,TInterpolationType.itLinear);
  FScreenRect.AnimateFloatDelay('Position.X',FParent.Width+10,1.5,0.3,TAnimationType.atIn,TInterpolationType.itQuartic);
...
begin
  ...
  begin
    ...
    lLabel.FontFill.Color := FMenuInactiveSubMenuColor;
...
begin
  FSubMenuRect.AnimateFloat('Opacity',0,0.2,TAnimationType.atIn,TInterpolationType.itLinear);
end;

procedure TLSNHeaderMenu.ShowSubMenus(aPositionX: single);
begin
  FSubMenuRect.Position.X := aPositionX;
  FSubMenuRect.AnimateFloatDelay('Opacity',1,0.5,0.2,TAnimationType.atIn,TInterpolationType.itLinear);
...
begin
  ...
  begin
    ...
    if FHeaderFontIsBold then
      lLabel.Font.Style := [TFontStyle.fsBold];
    lLabel.FontFill.Color := FMenuInactiveHeaderColor;
  ...
  // start animations
  FHeaderRect.AnimateFloat('Opacity',1,1,TAnimationType.atIn,TInterpolationType.itLinear);
...
begin
  ...
  TLabel(Sender).FontFill.Color := FMenuFocusedHeaderColor;
end;

procedure TLSNMenu.HeaderMouseLeave(Sender: TObject);
begin
  TLabel(Sender).ApplyStyleLookup;
  TLabel(Sender).FontFill.Color := FMenuInactiveHeaderColor;
end;
...
begin
  ...
  //
  TLabel(Sender).ApplyStyleLookup;
  TLabel(Sender).FontFill.Color := FMenuActiveHeaderColor;
  //
...
begin
  TLabel(Sender).ApplyStyleLookup;
  TLabel(Sender).FontFill.Color := FMenuFocusedSubMenuColor;
end;

procedure TLSNMenu.SubMenuMouseLeave(Sender: TObject);
begin
  TLabel(Sender).ApplyStyleLookup;
  TLabel(Sender).FontFill.Color := FMenuInactiveSubMenuColor;
end;
...
begin
  //
  TLabel(Sender).ApplyStyleLookup;
  TLabel(Sender).FontFill.Color := FMenuActiveSubMenuColor;
  //
...
begin
  ...
  FContainer.AnimateFloatDelay('Opacity',0,0.2,0,TAnimationType.atIn,TInterpolationType.itLinear);
  FContainer.Position.X := FParent.Width+10;
...
begin
  ...
  case aAnimationType of
    ...
    begin
      ...
      FContainer.Position.X := FParent.Width + 10;
      FContainer.AnimateFloatDelay('Position.X',0,0.3,0,TAnimationType.atIn,TInterpolationType.itQuartic);
    end;
    else
    begin
      FContainer.Opacity := 0;
      FContainer.Position.X := 0;
      FContainer.AnimateFloatDelay('Opacity',1,0.2,0,TAnimationType.atIn,TInterpolationType.itLinear);
    end;
  end;
  //FContainer.Position.X := FParent.Width + 10;
  //
